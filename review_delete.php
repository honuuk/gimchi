...
if(isset($_GET['no']))
{
  $no = $_GET['no'];
	...
	{
    $sql = 'DELETE FROM review WHERE numbers='.$no;
    $result = $conn->query($sql);
    // 쿼리문이 제대로 수행되었다면
    if ($result)
    {
      echo '<script>alert("삭제되었습니다.");location.href="review.php";</script>';
      exit;
    }
    else
    {
      echo '<script>alert("삭제 실패");location.href="review.php";</script>';
      exit;
    }
	}
	else
	{
		echo '<script>alert("존재하지 않는 게시물입니다.");location.href="review.php";</script>';
    exit;
	}
