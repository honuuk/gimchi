...
if ($type == 'admin')
{
	...
	$sql = "SELECT count(*) as cnt FROM product ORDER BY p_id DESC";
	...
	if ($page != 1)
	{
		$paging .= '<li class="active"><a href="admin_product.php?page=1">처음</a></li>';
	}

	//첫 섹션이 아니라면 이전 버튼을 생성
	if ($currentSection != 1)
	{
		$paging .= '<li class="active"><a href="admin_product.php?page=' . $prevPage . '">이전</a></li>';
	}
	...
	for ($i = $firstPage; $i <= $lastPage; $i++)
	{
		...
		{
			$paging .= '<li class="active"><a href="admin_product.php?page=' . $i . '">' . $i . '</a></li>';
		}
	}
	...
	if ($currentSection != $allSection)
	{
		$paging .= '<li class="active"><a href="admin_product.php?page=' . $nextPage . '">다음</a></li>';
	}

	//마지막 페이지가 아니라면 끝 버튼을 생성
	if ($page != $allPage)
	{
		$paging .= '<li class="active"><a href="admin_product.php?page=' . $allPage . '">끝</a></li>';
	}
	...
	$sql = "SELECT * FROM product ORDER BY p_id DESC".$sqlLimit;
...
?>
    ...
    <form id="joinForm" name="joinForm" action="admin_product_delete.php" method="get" enctype="multipart/form-data" >
      <div id="join">
        <div class="top">
          <h2>관리자페이지</h2>
        ...
        <div id="join-form">
    			...
    			<div id="form_inner">
                  ...
                  <?php
            			...
            			?>
            			<tr>
            				<td><input type="checkbox" name="pid<?php echo $i; ?>" value="<?php echo $id; ?>"></td>
										<td><?php echo $id; ?></td>
