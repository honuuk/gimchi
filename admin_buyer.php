...
if ($type == 'admin')
{
	...
	$sql = "SELECT count(*) as cnt FROM buyer ORDER BY name";
	...
	if ($page != 1)
	{
		$paging .= '<li class="active"><a href="admin_buyer.php?page=1">처음</a></li>';
	}

	//첫 섹션이 아니라면 이전 버튼을 생성
	if ($currentSection != 1)
	{
		$paging .= '<li class="active"><a href="admin_buyer.php?page=' . $prevPage . '">이전</a></li>';
	}
	...
	for ($i = $firstPage; $i <= $lastPage; $i++)
	{
		...
		{
			$paging .= '<li class="active"><a href="admin_buyer.php?page=' . $i . '">' . $i . '</a></li>';
		}
	}
	...
	if ($currentSection != $allSection)
	{
		$paging .= '<li class="active"><a href="admin_buyer.php?page=' . $nextPage . '">다음</a></li>';
	}

	//마지막 페이지가 아니라면 끝 버튼을 생성
	if ($page != $allPage)
	{
		$paging .= '<li class="active"><a href="admin_buyer.php?page=' . $allPage . '">끝</a></li>';
	}
	...
	$sql = "SELECT * FROM buyer ORDER BY name".$sqlLimit;
...
?>
    ...
    <form id="joinForm" name="joinForm" action="admin_buyer_delete.php" method="get" enctype="multipart/form-data" >
      <div id="join">
        <div class="top">
          <h2>관리자페이지</h2>
        ...
        <div id="join-form">
    			...
    			<div id="form_inner">
                  ...
                  <?php
            			...
            			?>
            			<tr>
            				<td><input type="checkbox" name="bid<?php echo $i; ?>" value="<?php echo $id; ?>"></td>
            				<td><?php echo $id; ?></td>
										<td><?php echo $name; ?></td>
