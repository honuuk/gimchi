...
if(isset($_GET['no']))
{
	...
	$sql = 'SELECT * FROM review WHERE numbers='.$no;
	$result = $conn->query($sql);
	...
	{
		echo '<script>alert("접근 권한이 없습니다.");location.href="review.php";</script>';
		exit;
	}
	}
	else
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		...
		else
		{
			...
			$sql = 'UPDATE review SET title="'.$title.'", content="'.$content.'" WHERE numbers='.$no;
			...
			if($result)
			{
				...
				$replaceURL = './reivew_view.php?no='.$no;

				echo '<script>alert("정상적으로 글이 수정되었습니다.");location.href="review.php";</script>';
											...
											<tbody>
												<tr>
													...
													<td>
															...
															<input type="text" name="title" class="txt-field" value="<?php echo $title; ?>">
